...
fn longer_valid_chain_replaces_current() {
    ...
    let b1 = Block::mine(&local.tip(), vec![tx(b"a")], 2, 1);
    local.blocks.push(b1.clone());

    let mut candidate = Blockchain::init();
    candidate.blocks.push(b1);

    let b2 = Block::mine(&candidate.tip(), vec![tx(b"b")], 2, 2);
...
fn shorter_chain_is_rejected() {
    ...
    let b1 = Block::new(1, local.tip().block_hash.clone(), 0, vec![tx(b"a")], 0, 2);
    local.blocks.push(b1.clone());
...
fn invalid_chain_is_rejected_with_error() {
    ...
    let mut bad = Block::new(
        1,
        candidate.tip().block_hash.clone(),
        0,
        vec![tx(b"a")],
        0,
        2,
