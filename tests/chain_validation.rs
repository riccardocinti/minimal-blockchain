...
fn valid_chain_passes_validation() {
    ...
    let b1 = Block::mine(&chain.tip(), vec![tx(b"a")], 2, 1);
    let b2 = Block::mine(&b1, vec![tx(b"b")], 2, 2);
...
fn chain_with_invalid_height_fails() {
    ...
    let b1 = Block::new(2, chain.tip().block_hash.clone(), 0, vec![tx(b"a")], 0, 2);
...
fn chain_with_broken_hash_link_fails() {
    ...
    let b1 = Block::new(1, "fake".into(), 0, vec![tx(b"a")], 0, 2);
...
fn chain_with_tampered_block_fails() {
    ...
    let mut b1 = Block::new(1, chain.tip().block_hash.clone(), 0, vec![tx(b"a")], 0, 2);
...
#[test]
fn chain_with_invalid_block_timestamp_fails() {
    let mut chain = Blockchain::init();

    let b1 = Block::mine(&chain.tip(), vec![tx(b"a")], 2, 1);
    let mut b2 = Block::mine(&b1, vec![tx(b"b")], 2, 2);
    b2.timestamp = 0;
